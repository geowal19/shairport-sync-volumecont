                           ...
                           int frame_count_max)
{
    ...
    if (frame_count_min > fill_count)
    {
        ...
        if ((err = soundio_outstream_end_write(outstream))) debug(0, "[--->>] end write error: %s", soundio_strerror(err));
    ...
    while (frames_left > 0)
    {
        ...
        if ((err = soundio_outstream_begin_write(outstream, &areas, &frame_count))) debug(0, "[--->>] begin write error: %s", soundio_strerror(err));
...
static void start(int sample_rate, int sample_format)
{
    ...
    if (outstream->layout_error) debug(0, "unable to set channel layout: %s\n", soundio_strerror(outstream->layout_error));

    int capacity = outstream->sample_rate * outstream->bytes_per_frame;

    ring_buffer = soundio_ring_buffer_create(soundio, capacity);

    if (!ring_buffer) debug(0, "unable to create ring buffer: out of memory");
...
{
    printf(" There are no options for libsoundio.\n");
}
