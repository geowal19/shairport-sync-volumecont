...
int jack_init(__attribute__((unused)) int argc, __attribute__((unused)) char * * argv)
{
    ...
    if (config.cfg != NULL)
    {
        ...
        if (config_lookup_int(config.cfg, "jack.bufsz", &bufsz) && bufsz <= 0) die("jack: bufsz must be > 0");
    }

    if (config.jack_client_name == NULL) config.jack_client_name = strdup("shairport-sync");
    ...
    if (jackbuf == NULL) die("Can't allocate %d bytes for the JACK ringbuffer.", bufsz);
...
void jack_deinit()
{
    ...
    if (jack_deactivate(client)) warn("Error deactivating jack client");

    if (jack_client_close(client)) warn("Error closing jack client");
    ...
    for (i = 0; i < 2; ++i)
    {
        ...
        if (soxr)
        {
            ...
            while (samples > 0 && thisbuf > 0)
            {
                ...
                if (e) die("Error during soxr process: %s", e);
